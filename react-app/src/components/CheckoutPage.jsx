  ...
  const handlePlaceOrder = () => {
    ...
    fetch('http://localhost:5000/orders', {
      ...
      .then(response => {
        ...
        return response.json();  // Return the data if you want to use it
      })
      .then(data => {
        console.log(data);  // You can log or use the data here
        onPlaceOrder();  // Call the parent callback to clear the cart or redirect
      })
      .catch(err => {
...
CheckoutPage.propTypes = {
  ...
  })).isRequired,  // cart must be an array of objects
  onPlaceOrder: PropTypes.func.isRequired   // onPlaceOrder must be a function
};

// Optionally, add default props if required
CheckoutPage.defaultProps = {
  cart: [],  // Default cart is an empty array
  onPlaceOrder: () => {},  // Default to an empty function
};
